    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                ...
                @if(\Carbon\Carbon::today() ==  \Carbon\Carbon::parse($borrow->returnAt)) 
                <a target="_blank" class="badge bg-primary" href="https://wa.me/{{$borrow->member->phone}}?text=Hallo {{ $borrow->member->name }}, saya dari perpustakaan Bank Indonesia Purwokerto.%0D%0A%0D%0AWaktu peminjamanmu dengan judul  hari ini sudah mencapai tenggat waktu.%0D%0A%0D%0AJangan lupa untuk mengembalikan buku ke Perpustakaan Bank Indonesia Purwokerto atau melakukan perpanjangan dengan melakukan konfirmasi ke nomor ini. Terima Kasih"><i class="ri-chat-3-fill"></i></a>
                @elseif(\Carbon\Carbon::today() >=  \Carbon\Carbon::parse($borrow->returnAt))
                <a target="_blank" class="badge bg-primary" href="https://wa.me/{{$borrow->member->phone}}?text=Hallo {{ $borrow->member->name }}, saya dari perpustakaan Bank Indonesia Purwokerto.%0D%0A%0D%0AWaktu peminjamanmu sudah terlewat {{ \Carbon\Carbon::today()->diffInDays($borrow->returnAt) }} hari.%0D%0A%0D%0AJangan lupa untuk mengembalikan buku ke Perpustakaan Bank Indonesia Purwokerto. Terima Kasih"><i class="ri-chat-3-fill"></i></a>
                @else 
                <a target="_blank" class="badge bg-primary" href="https://wa.me/{{$borrow->member->phone}}?text=Hallo {{ $borrow->member->name }}, saya dari perpustakaan Bank Indonesia Purwokerto.%0D%0A%0D%0AWaktu peminjamanmu tersisa {{ \Carbon\Carbon::today()->diffInDays($borrow->returnAt) }} hari lagi.%0D%0A%0D%0AJangan lupa untuk mengembalikan buku ke Perpustakaan Bank Indonesia Purwokerto. Terima Kasih"><i class="ri-chat-3-fill"></i></a>
                @endif
              </td>
              <td>
                @if($borrow->status=="borrowed")
  ...
  <tbody>
      ...
      <tr>
          ...
          <td>
              ...
              <a  class="badge bg-primary" href="https://wa.me/{{$return->member->phone}}?text=Hallo {{$return->member->name}} 👋, Saya {{auth()->user()->name}} dari Perpustakaan Bank Indonesia Purwokerto mengingatkan bahwa masa peminjaman buku tersisa {{\Carbon\Carbon::today()->diffInDays($return->returnAt)}} hari lagi. Jangan lupa untuk mengembalikan buku ke Perpustakaan Bank Indonesia Purwokerto segera~ 💫"><i class="ri-chat-3-fill"></i></a>
